...
fun PostponedArgumentsAnalyzer.Context.addSubsystemFromExpression(expression: FirExpression) {
    when (expression) {
        is FirFunctionCall, is FirCallLikeControlFlowExpression -> (expression as FirCalleeReferenceHolder).candidate()?.let { addOtherSystem(it.system.asReadOnlyStorage()) }
...
internal fun FirCalleeReferenceHolder.candidate(): Candidate? {
