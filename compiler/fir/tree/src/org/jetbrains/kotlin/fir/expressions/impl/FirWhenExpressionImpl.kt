...
class FirWhenExpressionImpl(
    ...
    override fun <D> transformBranches(transformer: FirTransformer<D>, data: D): FirElement {
        branches.transformInplace(transformer, data)
        return this
    }

    override fun <D> transformChildrenWithoutBodies(transformer: FirTransformer<D>, data: D): FirElement {
        transformSubject(transformer, data)
        branches.transformInplace { branch ->
            (branch.transformCondition(transformer, data) as FirWhenBranch).compose()
        }
        return super.transformChildren(transformer, data)
    }

    override fun <D> transformSubject(transformer: FirTransformer<D>, data: D): FirElement {
